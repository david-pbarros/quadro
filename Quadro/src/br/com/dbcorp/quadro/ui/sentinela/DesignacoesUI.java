...
import br.com.dbcorp.quadro.gerenciador.SentinelaGerenciador;
...
public class DesignacoesUI extends InternalUI implements ActionListener, ItemListener {
	...
	private SentinelaGerenciador gerenciador;
	...
	private List<Sentinela> sentinelaMes;
	...
	public DesignacoesUI() {
		...
		this.gerenciador = new SentinelaGerenciador();
	...
	private void montaMes() {
		...
		this.sentinelaMes = new ArrayList<Sentinela>();
		...
		for (DiaReuniao diaReuniao : this.diasReuniao) {
			Sentinela sentinela = this.gerenciador.obterSentinelaSemana(diaReuniao);
			this.sentinelaMes.add(sentinela);
			
			SemanaUI semana = new SemanaUI(diaReuniao, sentinela, this.pessoas);
	...
	private void salvar() {
		List<Sentinela> estudos = new ArrayList<Sentinela>();
		
		for (SemanaUI semana : this.semanas) {
			estudos.add(semana.obterSentinela());
		}
		
		this.gerenciador.salvarSentinela(estudos);
	...
	private void imprimir() {
		ReportCommon.getInstance().gerarSentinela(this.sentinelaMes, this.mesDesignacao);
	}
}